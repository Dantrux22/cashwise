...
let _splitListeners={}; // {groupId: unsubscribe function}
let _splitCollectionListeners=[]; // collection-level onSnapshot unsubscribers
...
async function onUserLoggedIn(user){
  ...
  // Set up real-time collection listeners for split groups (own + shared by uid + shared by email)
  setupSplitGroupListeners(user);
}
...
function listenSplitGroup(groupId){
  if(_splitListeners[groupId]) return;
    ...
    .onSnapshot(doc=>{
      ...
      if(idx!==-1){
        ...
        if(detail&&!detail.classList.contains('hidden')){
          renderSplitDetail(groupId);
        }
        renderSplitContent();
      }
    },err=>{ console.error('listenSplitGroup error:',err); });
  _splitListeners[groupId]=unsubscribe;
}

function stopListenSplitGroup(groupId){
  if(_splitListeners[groupId]){
    _splitListeners[groupId]();
    delete _splitListeners[groupId];
  }
}

function stopAllSplitListeners(){
  Object.keys(_splitListeners).forEach(stopListenSplitGroup);
}

// ── Grupos Split compartidos — listeners en tiempo real ──

function stopAllSplitCollectionListeners(){
  _splitCollectionListeners.forEach(fn=>fn());
...
// Sets up three persistent onSnapshot collection listeners after login:
//   1. Groups this user owns (covers new-device logins)
//   2. Groups where their uid is in sharedWith (already-linked members)
//   3. Groups where their email is in sharedWithEmails (pre-registration invites)
// All three stay active so new shares are picked up in real time without a re-login.
function setupSplitGroupListeners(user){
  if(!FIREBASE_ENABLED||!_fbDb||!user) return;
