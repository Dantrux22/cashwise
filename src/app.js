...
function drawNetCanvas(txs, period) {
  ...
  // Ingresos acumulados (verde claro, debajo) → neto acumulado (verde vivo, encima)
  drawNetLine(incAccum, 'rgba(52,212,138,0.5)', 'rgba(52,212,138,0.08)');
  drawNetLine(netAccum, '#34D48A', 'rgba(52,212,138,0.15)');
}
...
function parseMoneeExcel(arrayBuffer) {
  ...
  function findCatName(moneecat) {
    const allCats = [...(S.cats.expense||[]),...(S.cats.income||[]),...(S.cats.invest||[])];
    const exact = allCats.find(c => c.n.toLowerCase() === moneecat.toLowerCase());
  ...
  for(let i = 1; i < rows.length; i++) {
    ...
    const cat = findCatName(String(category || '').trim());
