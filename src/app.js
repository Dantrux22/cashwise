...
function drawNetCanvas(txs, period) {
  ...
  if(!incData.length && !expData.length) return;
  let cum = 0;
  const netData = incData.map((inc, i) => { cum += inc - (expData[i]||0); return cum; });
  const n = netData.length;
  const minVal = Math.min(...netData, 0);
  const maxVal = Math.max(...netData, 0);
  ...
  ctx.lineTo(xp(0), yp(netData[0]));
  for(let i=1; i<n; i++) {
    const cpx = (xp(i-1)+xp(i))/2;
    ctx.bezierCurveTo(cpx, yp(netData[i-1]), cpx, yp(netData[i]), xp(i), yp(netData[i]));
  }
  ...
  ctx.beginPath();
  ctx.arc(xp(n-1), yp(netData[n-1]), 4, 0, Math.PI*2);
  ctx.fillStyle = color; ctx.fill();
}
