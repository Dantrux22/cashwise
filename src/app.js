...
function drawCanvas(txs, period) {
  ...
  function drawLine(data, color) {
    ...
    ctx.closePath();
    ctx.fillStyle = color === '#34D48A' ? 'rgba(52,212,138,0.15)' : 'rgba(240,86,106,0.15)';
    ctx.fill();
    // Línea
    ctx.beginPath();
  ...
  ctx.font = `${Math.max(9, 10*(w/390))}px DM Sans, sans-serif`;
  ctx.textAlign = 'center';
  const step = Math.ceil(n/6);
  for(let i=0; i<n; i+=step) ctx.fillText(labels[i], xp(i), h-6);
  if((n-1)%step !== 0) ctx.fillText(labels[n-1], xp(n-1), h-6);
}
