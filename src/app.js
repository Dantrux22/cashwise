...
function drawCanvas(canvasId, incomeVals, expVals, labels, H){
  ...
  function drawArea(vals,color,alpha){
    ...
    pts.forEach(p=>{
      ...
      ctx.beginPath(); ctx.arc(p.x,p.y,2,0,Math.PI*2); ctx.fillStyle='#0f0f13'; ctx.fill();
    });
  }
  drawArea(expVals,'#f0566a',0.32);
  drawArea(incomeVals,'#34d48a',0.28);
...
function initAuth(){
  ...
  // Manejar resultado de redirect (Google sign-in desde redirect)
  _fbAuth.getRedirectResult().then(result=>{
    if(result && result.user){
      console.log('[finflow] Redirect result: usuario logueado');
    }
  }).catch(e=>{
    if(e.code && e.code !== 'auth/no-auth-event'){
      console.warn('[finflow] Redirect error:', e.code);
    }
  });
...
async function authGoogle(){
  if(!FIREBASE_ENABLED){ showAuthError('Firebase no configurado. Pegá tus claves.'); return; }
  const provider = new firebase.auth.GoogleAuthProvider();
  provider.setCustomParameters({ prompt: 'select_account' });
  // file:// no soporta OAuth — informar al usuario
  if(location.protocol === 'file:'){
    showAuthError('Para login con Google abrí el archivo desde un servidor. Usá "Usar sin cuenta" o el login con email.');
    return;
  }
  try{
    // Intentar popup primero, fallback a redirect
    await _fbAuth.signInWithPopup(provider);
    clearAuthError();
  }catch(e){
    if(e.code==='auth/popup-blocked'||e.code==='auth/popup-cancelled'||e.code==='auth/operation-not-supported-in-this-environment'){
      try{ await _fbAuth.signInWithRedirect(provider); }
      catch(e2){ showAuthError(authErrorMsg(e2.code)); }
