...
// Única categoría de inversión donde tiene sentido elegir moneda/cotización
// (comprar/vender otra divisa). El resto (Acciones, Cripto, etc.) queda
// siempre en tu moneda principal, sin etiqueta de moneda.
const CURRENCY_PURCHASE_CAT_NAME='Compra de monedas';
...
function saveTx(){
  ...
  const enToggle=document.getElementById('exclude-net-toggle');
  const enRow=document.getElementById('exclude-net-row');
  // Mismo toggle, dos significados según la categoría: "Excluir del neto"
  // para Acciones/Cripto/etc., o "Ya la tenía antes" para Compra de monedas
  // (ver _updateInvestRateRow) — en ambos casos, si está activo, no resta del balance.
  ...
  if(txType==='invest'){
    ...
    // Moneda y cotización solo aplican a "Compra de monedas" — el resto de
    // las inversiones (Acciones, Cripto, etc.) queda en tu moneda principal,
    // sin etiqueta de moneda, aunque hayas tenido otra moneda seleccionada
    // en una carga anterior dentro de la misma sesión.
    if(selCat===CURRENCY_PURCHASE_CAT_NAME){
      ...
      // está visible, es decir, la moneda elegida es distinta a la principal).
      // El total en pesos se calcula (cotización × cantidad) donde haga
      // falta, nunca se guarda como número fijo aparte.
...
function _updateTxCurrencyToggle(){
  ...
  const currLbl=document.getElementById('add-curr-lbl');
  if(!row) return;
  // Solo tiene sentido elegir moneda para "Compra de monedas" — el resto de
  // las inversiones (Acciones, Cripto, etc.) queda en tu moneda principal.
  if(txType!=='invest'||selCat!==CURRENCY_PURCHASE_CAT_NAME){
    row.style.display='none';
    if(currLbl) currLbl.textContent=S.currency.code;
    return;
  }
  row.style.display='';
  ...
  // El label chiquito arriba del monto sigue a la moneda elegida (ej "USD"),
  // no se queda pegado en tu moneda principal.
  if(currLbl) currLbl.textContent=txCurrency;
...
// Toggle "Excluir del neto" — para Compra de monedas cambia de significado
// a "Ya la tenía antes" (ver _updateInvestRateRow) y además esconde la
// cotización, ya que no tiene sentido pedir un precio de algo que no
// compraste ahora. Hay que re-evaluar la fila de cotización al tocarlo.
function toggleExcludeNet(el){
  el.classList.toggle('on');
  _updateInvestRateRow();
}

// Fila opcional "cotización por unidad" — solo tiene sentido para Compra de
// monedas, en una moneda distinta a la principal, y cuando NO tocaste "Ya la
// tenía antes". Si se completa, ese monto es lo que afecta tu balance en tu
// moneda principal; si se deja vacío, la inversión no toca el balance.
function _updateInvestRateRow(){
  const row=document.getElementById('invest-rate-row');
  const enRow=document.getElementById('exclude-net-row');
  const enLbl=document.getElementById('exclude-net-lbl');
  const enSub=document.getElementById('exclude-net-sub');
  const enToggle=document.getElementById('exclude-net-toggle');
  if(!row) return;
  const isCurrencyPurchase=txType==='invest'&&selCat===CURRENCY_PURCHASE_CAT_NAME;
  const isForeign=isCurrencyPurchase&&txCurrency&&txCurrency!==S.currency.code;
  const hadBefore=!!(enToggle&&enToggle.classList.contains('on'));

  if(txType==='invest'&&enRow) enRow.style.display='';
  else if(enRow) enRow.style.display='none';

  if(isCurrencyPurchase){
    // Para Compra de monedas, el toggle pasa a significar "ya tenía esto
    // desde antes" — no es una compra nueva, así que tampoco pide cotización.
    if(enLbl) enLbl.textContent='Ya la tenía antes';
    if(enSub) enSub.textContent='No es una compra nueva — no resta de tu balance';
  } else {
    if(enLbl) enLbl.textContent='Excluir del neto';
    if(enSub) enSub.textContent='Esta plata sigue siendo tuya';
  }

  row.style.display=(isForeign&&!hadBefore)?'':'none';
  if(!isForeign||hadBefore) return;
...
function _renderSecondaryRow(){
  ...
  if(!row) return;
  const mainCode=S.currency.code;
  // Solo "Compra de monedas" — plata que genuinamente tenés en otra moneda.
  // Acciones/Cripto/etc. compradas con otra moneda NO suman acá: son un
  // activo, no la moneda en sí (evita que "comprar acciones en USD" infle
  // este total como si fueran dólares que tenés disponibles).
  const foreignInvest=S.txs.filter(t=>t.type==='invest'&&t.currency&&t.currency!==mainCode&&t.cat===CURRENCY_PURCHASE_CAT_NAME);
...
function buildCatCircle(cat, type, isFreq){
  ...
  el.onclick=()=>{
    if(editingId){
      ...
      renderTxCatCircles(txType);
      if(type==='invest'){ _updateTxCurrencyToggle(); _updateInvestRateRow(); }
      return;
    }
