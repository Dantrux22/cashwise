...
// Gráfico dual: ingresos y gastos por período (sin acumulado)
...
function drawNetCanvas(txs, period) {
  ...
  // Acumulados
  let incCum = 0, netCum = 0;
  const incAccum = incData.map(inc => { incCum += inc; return incCum; });
  const netAccum = incData.map((inc, i) => { netCum += inc - (expData[i]||0); return netCum; });

  const n = incAccum.length;
  const minVal = Math.min(...netAccum, 0);
  const maxVal = Math.max(...incAccum, ...netAccum, 1);
  ...
  reader.onload=e=>{
    try{
      ...
      showConfirm('Restaurar backup','Esto reemplazará todos tus datos actuales.',()=>{
        ...
        if(!S.split||typeof S.split!=='object') S.split={groups:[],expenses:[]};
...
function initSplit(){
  ...
  renderSplitContent();
  // Refresh from Firestore every time the user opens the Split screen
  refreshSplitGroups();
}
  ...
  if(editingGroupId){
    ...
    if(g){
      ...
      if(FIREBASE_ENABLED&&_fbDb&&_authUser){
        ...
        _fbDb.collection('splitGroups').doc(g.id).set(firestoreDoc)
          .then(()=>{ console.log('[Split] Group updated OK:', firestoreDoc.id); refreshSplitGroups(); })
    ...
    if(FIREBASE_ENABLED&&_fbDb&&_authUser){
      ...
      _fbDb.collection('splitGroups').doc(newGroup.id).set(firestoreDoc)
        .then(()=>{ console.log('[Split] Group saved OK:', firestoreDoc.id); showToast('☁️ Grupo sincronizado en la nube'); refreshSplitGroups(); })
...
// ── Split: no persistent listeners — manual refresh only ──
// These stubs exist so callers elsewhere don't break.
function listenSplitGroup(groupId){ /* no-op: using manual refresh instead of onSnapshot */ }
function stopListenSplitGroup(groupId){ /* no-op */ }
function stopAllSplitListeners(){ /* no-op */ }
function stopAllSplitCollectionListeners(){ /* no-op */ }
...
// Merges one Firestore doc into S.split.groups (Firestore is source of truth).
// Also backfills uid in Firestore if the member was added by email only.
function _handleSplitGroupDoc(doc, user){
  ...
  const group={...remoteGroup,members};
  const idx=S.split.groups.findIndex(g=>g.id===group.id);
  if(idx===-1) S.split.groups.push(group);
  else S.split.groups[idx]=group;
  // Backfill uid in Firestore if needed (handles re-login with persisted state)
  if(user.email){
...
// Fetches all split groups for the current user from Firestore (three queries,
// deduplicated) and updates local state. Called on login, screen navigation,
// and after any write operation.
async function refreshSplitGroups(){
  ...
  try{
    ...
    const queries=[
      db.where('ownerUid','==',user.uid).get(),
      db.where('sharedWith','array-contains',user.uid).get(),
    ];
    if(user.email) queries.push(
      db.where('sharedWithEmails','array-contains',user.email.toLowerCase()).get()
    );
    ...
    // Remove groups that no longer exist in Firestore (deleted or access revoked)
    // Keep local-only groups (no ownerUid) as they haven't been synced yet
    const remoteIds=new Set(docs.map(d=>d.id));
    S.split.groups=S.split.groups.filter(g=>!g.ownerUid||remoteIds.has(g.id));
    // Process each doc: update group + expenses
    docs.forEach(d=>{
      _handleSplitGroupDoc(d,user);
      // Sync expenses from the group doc field into S.split.expenses
      const data=d.data();
      if(Array.isArray(data.expenses)){
        S.split.expenses=S.split.expenses.filter(e=>e.groupId!==d.id);
        S.split.expenses.push(...data.expenses);
      }
    });
    ...
    console.error('[Split] refreshSplitGroups error:',e);
    if(e.code==='permission-denied')
      console.error('[Split] Check Firestore security rules for splitGroups collection');
  }finally{
